...
export var Client = function(wsdl, endpoint) {
    this.wsdl = wsdl;
...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    // Hack to get in the correct SOAPAction - don't judge me.
    if (_.values(_.get(defs, 'bindings', [])).length == 1) {
      var realMethod = _.get(_.values(_.get(defs, 'bindings'))[0], 'methods.' + name, false);
      if( realMethod )
        headers.SOAPAction = realMethod.soapAction;
    }
