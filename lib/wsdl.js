...
DefinitionsElement.prototype.addChild = function(child) {
    ...
    if (child instanceof TypesElement) {
      ...
      // Do nothing... Copied from another fork...
    }
    else {
...
WSDL.prototype._processNextInclude = function(includes, callback) {
    ...
    open_wsdl(url.urlResolve(self.uri, include.location), function(err, wsdl) {
...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, xmlns) {
    ...
    function append_ns(name_space, obj) {
        // append the ns to the keys - use this type of for to avoid ES6
        var appended = {}
        for (var key in obj) {
            var value = obj[key];

            // if value is an object, go deper and append ns to children - ignore if object is empty
            if (typeof value === 'object' && value !== null && Object.keys(obj).length !== 0) {
                value = append_ns(name_space, value)
            }

            // otherwise, do normal appending
            appended[name_space + ':'+ key] = value;
        }

        return appended;
    }

    args[name] = append_ns(ns, params);
...
export function open_wsdl(uri, options, callback) {
